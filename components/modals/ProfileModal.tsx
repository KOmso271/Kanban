...
export default function ProfileModal({ 
  ...
  // NÚT LƯU 2: ĐỔI MẬT KHẨU
  const handleUpdatePassword = async () => {
    if (!oldPassword || !newPassword || !confirmPassword) {
      toast.error("Vui lòng điền đầy đủ các trường mật khẩu!"); return;
    }
    ...
    if (!passwordRegex.test(newPassword)) {
      toast.error("Mật khẩu phải có ít nhất 6 ký tự, bao gồm chữ hoa, thường, số và ký tự đặc biệt.", { duration: 4000 }); 
      return;
    }
    ...
    try {
      ...
      if (!user?.email) { toast.error("Không xác định được tài khoản!"); setIsSavingPassword(false); return; }

      const { error: signInError } = await supabase.auth.signInWithPassword({ email: user.email, password: oldPassword });
      if (signInError) { toast.error("Mật khẩu hiện tại không chính xác!"); setIsSavingPassword(false); return; }
    ...
    } catch (err) { toast.error("Đã xảy ra lỗi hệ thống!"); }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 transition-opacity" onClick={onClose}>
      <div className="bg-white dark:bg-[#15171C] w-full max-w-[420px] max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col scale-in-center scrollbar-hide" onClick={e => e.stopPropagation()}>
        
        {/* HEADER MODAL */}
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-[#1C1F26] sticky top-0 z-10">
        ...
        {/* MENU TABS */}
        <div className="flex border-b border-slate-100 dark:border-slate-800 px-6 pt-2 bg-slate-50/30 dark:bg-[#1C1F26]/30 overflow-x-auto scrollbar-hide">
          <button onClick={() => setActiveTab("profile")} className={`pb-2.5 text-[13px] font-medium transition-colors mr-5 flex-shrink-0 relative ${activeTab === 'profile' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300'}`}>
            {t.personalProfile}
            {activeTab === 'profile' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 dark:bg-blue-400 rounded-t-full"></span>}
          </button>
          <button onClick={() => setActiveTab("security")} className={`pb-2.5 text-[13px] font-medium transition-colors mr-5 flex-shrink-0 relative ${activeTab === 'security' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300'}`}>
            {t.securityTab}
          ...
          {isSuperAdmin && (
            <button onClick={() => setActiveTab("advanced")} className={`pb-2.5 text-[13px] font-medium transition-colors flex-shrink-0 relative ${activeTab === 'advanced' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300'}`}>
              Cài đặt nâng cao
              {activeTab === 'advanced' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 dark:bg-blue-400 rounded-t-full"></span>}
            </button>
          )}
        </div>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-2 duration-200">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowOldPassword(!showOldPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors">
          ...
          {activeTab === "advanced" && isSuperAdmin && allUsers && (
            <div className="animate-in fade-in slide-in-from-right-2 duration-200">
              <div className="rounded-xl border border-red-200 bg-red-50/30 p-5 dark:border-red-900/30 dark:bg-red-900/10">
                ...
                <p className="mb-4 text-[12px] text-red-500 dark:text-red-400">
                  Hành động chuyển quyền quản trị tối cao không thể hoàn tác. Bạn sẽ mất toàn bộ quyền truy cập cấp cao ngay lập tức sau khi xác nhận.
                </p>

                <div className="space-y-4">
                  {/* Chọn người kế nhiệm */}
                  <div>
                    <label className="mb-1 block text-[12px] font-semibold text-slate-700 dark:text-slate-300">Chọn người kế nhiệm</label>
                    <select 
                      value={selectedSuccessorId}
                      onChange={(e) => {
                        setSelectedSuccessorId(e.target.value);
                        setConfirmEmailInput(""); // Đổi người thì reset input xác nhận
                      }}
                      className="w-full rounded-md border border-red-200 bg-white px-3 py-2 text-[13px] outline-none focus:border-red-500 dark:border-red-800 dark:bg-[#1C1F26] dark:text-white transition-colors"
                    >
                      <option value="">-- Chọn thành viên --</option>
                      {allUsers.filter((u: any) => u.id !== myProfile?.id).map((u: any) => (
                        <option key={u.id} value={u.id}>{u.name} ({u.email})</option>
                      ))}
                    </select>
                  </div>

                  {/* Chốt chặn: Nhập email */}
                  {selectedSuccessorId && successor && (
                    <div className="animate-in fade-in slide-in-from-top-2">
                      <label className="mb-1 block text-[12px] font-semibold text-slate-700 dark:text-slate-300">
                        Gõ lại email <strong className="select-none text-red-600 bg-red-100 dark:bg-red-900/40 px-1 rounded">{successor.email}</strong> để xác nhận:
                      </label>
                      <input 
                        type="text" 
                        ...
                        autoComplete="off"
                        className="w-full rounded-md border border-red-300 bg-white px-3 py-2 text-[13px] outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 dark:border-red-700 dark:bg-[#15171C] dark:text-white transition-colors"
                      />
                      
                      <button 
                        onClick={() => onTransferSuperAdmin(selectedSuccessorId)}
                        disabled={!isEmailConfirmed}
                        className={`mt-4 w-full rounded-md py-2 text-[13px] font-bold text-white transition-all ${
                          isEmailConfirmed 
                            ? 'bg-red-600 hover:bg-red-700 shadow-md shadow-red-500/30 cursor-pointer' 
                            : 'bg-red-300 cursor-not-allowed dark:bg-red-900/50 dark:text-red-200/50'
                        }`}
                      >
                        TIẾN HÀNH CHUYỂN QUYỀN
                      </button>
                    </div>
                  )}
                </div>
