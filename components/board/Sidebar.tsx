...
export default function Sidebar({
  ...
  // 👇 CHỈ GIỮ LẠI STATE ĐỂ HIỂN THỊ TÊN CÔNG TY, XÓA CÁC STATE SỬA TÊN 👇
  ...
  useEffect(() => {
    const fetchCompanySettings = async () => {
      ...
      // Theo dõi Realtime nếu có ai đó đổi tên công ty từ Profile Modal
    ...
    // 👇 CÀI ĐẶT LẮNG NGHE REALTIME ĐỂ CẬP NHẬT TÊN NGAY LẬP TỨC 👇
    const companyChannel = supabase.channel('company_settings_changes')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'company_settings' }, (payload) => {
        setLocalCompanyName(payload.new.company_name);
      })
      .subscribe();

    return () => { supabase.removeChannel(companyChannel); };
  }, [currentUser]);
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-[240px] bg-slate-50 dark:bg-[#15171C] border-r border-slate-200 dark:border-slate-800 flex flex-col flex-shrink-0 transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      {/* 👇 KHỐI HIỂN THỊ TÊN CÔNG TY ĐÃ ĐƯỢC LÀM SẠCH (KHÔNG CÒN NÚT BẤM HAY MENU THẢ XUỐNG NỮA) 👇 */}
      <div className="relative h-14 px-4 flex items-center justify-between border-b border-slate-200/50 dark:border-slate-800/50 md:border-transparent">
        <div className="flex items-center px-2 py-1.5 flex-1 min-w-0">
          ...
          <div className="flex flex-col flex-1 min-w-0 pr-1.5">
            <span className="font-bold text-[14px] text-slate-800 dark:text-slate-200 truncate" title={localCompanyName}>{localCompanyName}</span>
        ...
        <button onClick={() => setIsSidebarOpen(false)} className="md:hidden p-1 text-slate-400 ml-1">
