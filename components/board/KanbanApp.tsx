...
export default function KanbanApp({ session }: { session: any }) {
  ...
  const t =
    translations[lang as keyof typeof translations] || translations["vi"];
  ...
  const [boardStats, setBoardStats] = useState<
    Record<string, { taskCount: number; progress: number }>
  >({});
  ...
  const activeBoardMembers = useMemo(() => {
    ...
    allBoardMembers
      .filter((bm) => bm.board_id === activeBoardId)
      .forEach((bm) => {
        const user = allUsers.find((u) => u.id === bm.user_id);
        if (user && !uniqueMembers.has(user.id))
          uniqueMembers.set(user.id, { ...user, role: bm.role });
      });
  ...
  const currentUserRole = useMemo(() => {
    ...
    const myMembership = allBoardMembers.find(
      (bm) => bm.board_id === activeBoardId && bm.user_id === currentUser.id,
    );
  ...
  const searchResults = useMemo(() => {
    ...
    return tasks.filter((task) =>
      task.content.toLowerCase().includes(searchTaskQuery.toLowerCase()),
    );
  }, [tasks, searchTaskQuery]);

  const filteredTasks = useMemo(() => {
    return tasks.filter((task) => {
      const safeIds = task.assignee_ids
      ...
      const matchesAssignee =
        filterAssigneeId === "all" || safeIds.includes(filterAssigneeId);
      const matchesLabel =
        filterLabelId === "all" ||
        taskLabels.some(
          (tl) => tl.task_id === task.id && tl.label_id === filterLabelId,
        );
      const matchesPriority =
        filterPriority === "all" ||
        (task.priority || "normal") === filterPriority;
  ...
  const executeTransferSuperAdmin = async () => {
    ...
    try {
      ...
      toast.success(
        "👑 Đã chuyển giao quyền lực thành công! Đang tải lại hệ thống...",
        { icon: "👑", duration: 4000 },
      );
  ...
  const handleExportFullBackup = async () => {
    ...
    try {
      ...
      const boardsSheetData =
        allBoards?.map((b) => ({
          "Mã Dự án": b.id,
          "Tên Dự án": b.name,
          "Độ ưu tiên": translatePriority(
            priorityConfig[b.priority || "normal"]?.label,
            lang,
          ),
      ...
      const tasksSheetData =
        allTasks?.map((t) => {
          const board = allBoards?.find(
            (b) =>
              allCols?.find((c) => c.id === t.column_id)?.board_id === b.id,
          );
          const col = allCols?.find((c) => c.id === t.column_id);
          const assignees = (
            Array.isArray(t.assignee_ids) ? t.assignee_ids : []
          )
          ...
          return {
            ...
            "Độ ưu tiên": translatePriority(
              priorityConfig[t.priority || "normal"]?.label,
              lang,
            ),
      ...
      XLSX.utils.book_append_sheet(
        workbook,
        XLSX.utils.json_to_sheet(boardsSheetData),
        "Danh sach Du an",
      );
      XLSX.utils.book_append_sheet(
        workbook,
        XLSX.utils.json_to_sheet(tasksSheetData),
        "Toan bo Cong viec",
      );
      XLSX.utils.book_append_sheet(
        workbook,
        XLSX.utils.json_to_sheet(usersSheetData),
        "Nhan su",
      );

      XLSX.writeFile(
        workbook,
        `Full_Backup_System_${format(new Date(), "ddMMyyyy")}.xlsx`,
      );
      toast.success("Đã xuất file sao lưu hệ thống thành công!", {
        id: "backup",
      });
    } catch (error) {
  ...
  useEffect(() => {
    const fetchMyBoards = async () => {
      if (isSuperAdmin) {
        ...
        const myBoardIds = allBoardMembers
          .filter((bm: any) => bm.user_id === currentUser.id)
          .map((bm: any) => bm.board_id);
  ...
  useEffect(() => {
    ...
    const adminBoardChannel = supabase
      .channel("super_admin_boards")
      .on(
        "postgres_changes",
      ...
      .subscribe();
    return () => {
      supabase.removeChannel(adminBoardChannel);
    };
  }, [isSuperAdmin]);
  ...
  useEffect(() => {
    const fetchBoardStats = async () => {
      ...
      const { data: allCols } = await supabase
        .from("columns")
        .select("id, board_id, order, title")
      ...
      boards.forEach((board) => {
        ...
        const bTasks = allTasks.filter((t) =>
          bCols.some((c) => c.id === t.column_id),
        );
        if (bTasks.length === 0 || bCols.length === 0) {
          stats[board.id] = { taskCount: bTasks.length, progress: 0 };
          return;
        }
        ...
        const lastCol =
          bCols.find((c) => {
            ...
            return (
              safeTitle.includes("done") ||
              safeTitle.includes("hoàn thành") ||
              safeTitle.includes("xong") ||
              safeTitle.includes("complete")
            );
          }) ||
          bCols.reduce((prev, current) =>
        ...
        stats[board.id] = {
          ...
          progress: Math.round((doneTasks.length / bTasks.length) * 100),
        };
      });
      setBoardStats(stats);
    };
  ...
  useEffect(() => {
    const dashboardChannel = supabase
      .channel("dashboard_global_changes")
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    const memberChannel = supabase
      .channel(`member_changes_${currentUser.id}`)
      .on(
        "postgres_changes",
        {
          event: "*",
        ...
        async (payload) => {
          ...
          if (payload.eventType === "INSERT") {
            ...
            if (boardInfo && boardInfo.user_id !== currentUser.id) {
              toast.success("Bạn vừa được mời vào một dự án mới!", {
                icon: "🎉",
              });
            }
  ...
  useEffect(() => {
    ...
    const tasksChannel = supabase
      .channel(`board_tasks_${activeBoardId}`)
      .on(
        "postgres_changes",
  ...
  const handleAddBoard = async (name: string, priority: string) => {
    ...
    try {
      const { data: newBoard, error } = await supabase
        .from("boards")
        .insert([{ name, priority, user_id: currentUser.id }])
        .select()
        .single();
      if (error) {
      ...
      if (newBoard) {
        const { error: memberError } = await supabase
          .from("board_members")
          .insert([
            { board_id: newBoard.id, user_id: currentUser.id, role: "admin" },
          ]);
        if (memberError) {
          toast.error(`Lỗi cấp quyền: ${memberError.message}`);
          return;
        }
        setBoards((prev) => [...prev, newBoard]);
        setAllBoardMembers((prev) => [
          ...prev,
          { board_id: newBoard.id, user_id: currentUser.id, role: "admin" },
        ]);
      ...
      toast.error("Đã xảy ra sự cố không xác định!");
    }
  };
  ...
  const handleAddMember = async (userId: string, role: string = "member") => {
    if (!activeBoardId) return;
    if (!isManager) {
      toast.error("Chỉ Quản trị viên mới được mời thêm người!");
      return;
    }
    const { data, error } = await supabase
      .from("board_members")
      .insert([{ board_id: activeBoardId, user_id: userId, role }])
      .select()
      .single();
    if (!error && data) {
  ...
  const handleRemoveMember = (userId: string) => {
    ...
    if (
      currentBoard &&
      userId === currentBoard.user_id &&
      currentUser.id !== currentBoard.user_id
    ) {
      if (!isSuperAdmin) {
        toast.error("Hỗn xược! Bạn không có quyền xóa Chủ dự án!");
        return;
      }
    }
    ...
    if (
      ...
      toast.error("Bạn không đủ thẩm quyền để xóa một Quản lý khác!");
      return;
    }
    if (!activeBoardId) return;
    if (!isManager) {
    ...
    if (isMe) {
      ...
      const myMembership = activeBoardMembers.find(
        (m: any) => m.id === currentUser.id,
      );
      const myRole = myMembership?.role?.toLowerCase() || "";
      const amIManager =
        myRole === "admin" || myRole === "quản lý" || myRole === "manager";

      // 👇 Chỉ chặn nếu TÔI LÀ QUẢN LÝ và tôi là người cuối cùng
      if (amIManager) {
        const adminCount = activeBoardMembers.filter(
          (m: any) =>
            m.role === "admin" || m.role === "Quản lý" || m.role === "manager",
        ).length;
        if (adminCount <= 1) {
          toast.error(
            "Bạn là Quản lý duy nhất! Hãy cấp quyền cho người khác trước khi rời đi, hoặc Xóa luôn dự án này.",
            { duration: 4000 },
          );
          return;
        }
  ...
  const executeRemoveMember = async () => {
    ...
    const { error } = await supabase
      .from("board_members")
      .delete()
      .match({ board_id: activeBoardId, user_id: userId });
    ...
    if (!error) {
          ...
          if (tasks) {
            for (const task of tasks) {
              ...
              if (Array.isArray(task.assignee_ids))
                currentAssigneeIds = [...task.assignee_ids];
              else if (typeof task.assignee_ids === "string") {
                try {
                  currentAssigneeIds = JSON.parse(task.assignee_ids);
                } catch (e) {
                  currentAssigneeIds = [];
                }
              }
              if (currentAssigneeIds.includes(userId)) {
                const newAssigneeIds = currentAssigneeIds.filter(
                  (id: string) => id !== userId,
                );
                await supabase
                  .from("tasks")
      ...
      setAllBoardMembers((prev) =>
        prev.filter(
          (bm) => !(bm.board_id === activeBoardId && bm.user_id === userId),
        ),
      );
      ...
      toast.error("Lỗi hệ thống khi thao tác!");
    }
  ...
  const handleChangeMemberRole = async (
    ...
    if (
      userId === currentUser.id &&
      (selectedRole === "member" || selectedRole === "Thành viên")
    ) {
      const adminCount = activeBoardMembers.filter(
        (m: any) =>
          m.role === "admin" || m.role === "Quản lý" || m.role === "manager",
      ...
      if (adminCount <= 1) {
        toast.error("Bạn là Quản lý duy nhất!", { duration: 4000 });
        return;
      }
    }
    const { error } = await supabase
    ...
    if (!error) {
      setAllBoardMembers((prev) =>
        prev.map((bm) =>
          bm.board_id === activeBoardId && bm.user_id === userId
            ? { ...bm, role: selectedRole }
            : bm,
        ),
      );
  ...
  const handleEditBoard = async (id: string, newName: string) => {
    if (!isManager) {
      toast.error("Chỉ Quản trị viên mới được đổi tên dự án!");
      return;
    }
    const { error } = await supabase
      .from("boards")
  ...
  const handleDeleteBoard = async (boardId: string) => {
    ...
    const isSuperAdmin =
      ...
      companyData[0].super_admin_id === currentUser.id;
    if (!isSuperAdmin) {
      toast.error("Chỉ Quản trị viên (Super Admin) mới được xóa dự án!");
      return;
    }
    ...
    if (!error) {
      ...
      toast.error("Lỗi khi xóa dự án dưới Database!");
    }
  };
  ...
  const handleChangePriority = async (id: string, priority: string) => {
    ...
    if (!error) {
      ...
      toast.success("Đã đổi độ ưu tiên!");
    }
  };
  ...
  const handleImportFile = async (
    ...
    reader.onload = async (e) => {
      try {
        ...
        const lines: any[][] = XLSX.utils.sheet_to_json(worksheet, {
          header: 1,
        });
        ...
        const parseExcelDate = (excelDate: any) => {
          ...
          if (!excelDate || val === "" || val.toLowerCase() === "không có")
            return null;
        ...
        if (!defaultColId) {
          toast.error("Dự án chưa có cột nào!");
          return;
        }
        ...
        for (let i = 1; i < validLines.length; i++) {
          ...
          if (colName) {
            const foundCol = columns.find(
              (c) => c.title.toLowerCase() === colName,
            );
            if (foundCol) colId = foundCol.id;
          }
          ...
          const assigneeName = row[2]
            ...
            : "";
          let assigneeId = null;
          if (assigneeName && assigneeName !== "chưa giao") {
            const foundMember = activeBoardMembers.find(
              (m: any) => m.name?.toLowerCase() === assigneeName,
            );
            if (foundMember) assigneeId = foundMember.id;
          }
        ...
        if (newTasksToInsert.length > 0) {
          const { data: insertedData, error } = await supabase
            ...
            .select();
          if (error) {
            toast.error(`Lỗi từ database: ${error.message}`);
          } else if (insertedData) {
            setTasks((prev: any) => [...prev, ...insertedData]);
            toast.success(
              `Đã tạo thành công ${insertedData.length} công việc!`,
            );
          }
        }
      } catch (error: any) {
  ...
  const handleExport = async (formatType: "xlsx" | "csv") => {
    if (filteredTasks.length === 0) {
      toast.error("Không có công việc nào để xuất!");
      return;
    }
    const XLSX = await import("xlsx");
    const exportData = filteredTasks.map((task) => {
      const safeIds = task.assignee_ids
        ? Array.isArray(task.assignee_ids)
          ...
          : [task.assignee_ids]
        : [];
      const assigneeNames = safeIds
        .map(
          (id: string) =>
            activeBoardMembers.find((m: any) => m.id === id)?.name,
        )
        .filter(Boolean);
      return {
        "Tên công việc": task.content,
        "Danh sách (Cột)":
          columns.find((c) => c.id === task.column_id)?.title || "Không rõ",
        "Người thực hiện":
          assigneeNames.length > 0 ? assigneeNames.join(", ") : "Chưa giao",
        "Độ ưu tiên": translatePriority(
          priorityConfig[task.priority || "normal"]?.label || "Bình thường",
          lang,
        ),
    ...
    const boardName =
      boards.find((b) => b.id === activeBoardId)?.name || "Du-an";
    XLSX.writeFile(
      workbook,
      `tien_do_${boardName}_${format(new Date(), "ddMMyyyy")}.${formatType}`,
    );
    setShowExportMenu(false);
  };

  const currentTaskLabelIds = selectedTask
    ? taskLabels
        .filter((tl) => tl.task_id === selectedTask.id)
        .map((tl) => tl.label_id)
    : [];
  ...
  const handleCreateLabel = async () => {
      ...
      .insert([
        {
          ...
          color: newLabelColor || "bg-blue-500 text-white",
        },
      ])
      .select()
      .single();
    if (newLabel) {
      ...
      setNewLabelName("");
      const { data: taskLabel } = await supabase
        .from("task_labels")
        .insert([{ task_id: selectedTask.id, label_id: newLabel.id }])
        .select()
        .single();
  ...
  const toggleLabelForTask = async (labelId: string) => {
    const exists = taskLabels.find(
      (tl) => tl.task_id === selectedTask.id && tl.label_id === labelId,
    );
    if (exists) {
      await supabase
        .from("task_labels")
        .delete()
        .match({ task_id: selectedTask.id, label_id: labelId });
      setTaskLabels((prev) =>
        prev.filter(
          (tl) => !(tl.task_id === selectedTask.id && tl.label_id === labelId),
        ),
  ...
  const handleTaskClick = useCallback((task: any) => {
    ...
    setEditTaskAssignees(
      rawIds ? (Array.isArray(rawIds) ? rawIds : [rawIds]) : [],
    );
  }, []);
  ...
  const handleAddChecklist = async () => {
    ...
    const { data } = await supabase
      ...
      .single();
    if (data) {
      setTaskChecklists((prev) => [...prev, data]);
      setNewChecklistTitle("");
      logActivity(selectedTask.id, "checklist", `đã thêm mục kiểm tra mới`);
    }
  };
  ...
  const toggleChecklist = async (id: string, currentStatus: boolean) => {
    ...
    setTaskChecklists((prev) =>
      prev.map((c) =>
        c.id === id ? { ...c, is_completed: !currentStatus } : c,
      ),
    );
    logActivity(
      selectedTask.id,
      "checklist",
      `đã đánh dấu ${!currentStatus ? "hoàn thành" : "chưa hoàn thành"} mục "${item?.title || "kiểm tra"}"`,
    );
  };
  ...
  const handleSendComment = async () => {
    ...
    if (data) {
      ...
      const notifyIds = selectedTask.assignee_ids
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { data } = await supabase
      ...
      .single();
    if (data) {
      setTaskAttachments((prev) => [data, ...prev]);
      toast.success("Tải tệp lên thành công!");
      logActivity(selectedTask.id, "attachment", `đã tải lên tệp đính kèm`);
    }
    setIsUploading(false);
  };
  ...
  const handleDeleteAttachment = async (id: string, fileUrl: string) => {
    ...
    if (fileName)
      await supabase.storage.from("task_attachments").remove([fileName]);
  ...
  const executeSaveColumn = async (title: string) => {
    const { data } = await supabase
      .from("columns")
      .insert([{ title, board_id: activeBoardId, order: columns.length + 1 }])
      .select()
      .single();
  ...
  const handleAddColumnKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (val && activeBoardId) {
        columnTitleRef.current = "";
        setNewColumnTitle("");
        setIsAddingColumn(false);
        executeSaveColumn(val);
      }
    }
    if (e.key === "Escape") {
      columnTitleRef.current = "";
  ...
  const requestDeleteTask = (id: string) => {
    if (!isManager) {
      toast.error("Chỉ Quản trị viên mới được phép xóa công việc này!");
      return;
    }
    setTaskToDelete(id);
  };
  ...
  const handleQuickComplete = async (taskId: string) => {
    ...
    const { data } = await supabase
      .from("tasks")
      .update({ column_id: targetColumn.id, priority: "low" })
      .eq("id", taskId)
      .select()
      .single();
    if (data) {
      ...
      logActivity(
        taskId,
        "move",
        `đã chuyển công việc sang cột "${targetColumn.title}"`,
      );
    }
  };
  ...
  const handleSaveTaskDetail = async () => {
    ...
    if ((selectedTask.description || "") !== editTaskDescription.trim())
      changes.push("mô tả");
    if ((selectedTask.start_date || null) !== finalStartDate)
      changes.push("ngày bắt đầu");
    if ((selectedTask.due_date || null) !== finalDueDate)
      changes.push("hạn chót");
    if ((selectedTask.priority || "normal") !== editTaskPriority)
      changes.push("độ ưu tiên");

    const oldAssignees = selectedTask.assignee_ids
      ? Array.isArray(selectedTask.assignee_ids)
        ? selectedTask.assignee_ids
        : [selectedTask.assignee_ids]
      : [];
    ...
    const assigneesChanged =
      ...
      !oldAssignees.every((id: string) => newAssignees.includes(id));
    ...
    const { data, error } = await supabase
      ...
      .update({
        ...
        assignee_ids: editTaskAssignees,
      })
    ...
    if (data) {
      setTasks((prev) =>
        prev.map((t) => (t.id === selectedTask.id ? data : t)),
      );
      const changeText = changes.join(", ");
      logActivity(selectedTask.id, "update", `đã cập nhật ${changeText}`);

      if (assigneesChanged) {
        const newlyAdded = newAssignees.filter(
  ...
  const onDragEnd = async (event: any) => {
    ...
    if (!isManager && activeType === "Column") {
      toast.error("Chỉ Quản trị viên mới được phép thay đổi vị trí Cột!");
      return;
    }

    if (activeType === "Column") {
      const overColumnId =
        overType === "Column" ? over.id : over.data.current?.task?.column_id;
    ...
    if (activeType === "Task") {
      ...
      if (oldColumnId !== overColumnId) {
        ...
        logActivity(
          activeTaskId,
          "move",
          `đã chuyển công việc sang cột "${targetCol?.title || "mới"}"`,
        );
      }

      let newTasks = [...tasks];
      if (tasks[oldIndex].column_id === overColumnId) {
        newTasks = arrayMove(
          newTasks,
          oldIndex,
  ...
  const completedChecklists = taskChecklists.filter(
    (c) => c.is_completed,
  ).length;
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-[#0E1116] overflow-hidden text-slate-800 dark:text-slate-200 relative transition-colors duration-300">
      ...
      <TaskDetailModal
        ...
        handleSaveTaskDetail={handleSaveTaskDetail}
        currentTaskLabels={boardLabels.filter(
          (l) =>
            selectedTask &&
            taskLabels.some(
              (tl) => tl.task_id === selectedTask.id && tl.label_id === l.id,
            ),
        )}
        boardLabels={boardLabels}
        currentTaskLabelIds={currentTaskLabelIds}
        showLabelMenu={showLabelMenu}
        setShowLabelMenu={setShowLabelMenu}
        toggleLabelForTask={toggleLabelForTask}
      ...
      <MembersModal
        ...
        currentBoard={boards.find((b) => b.id === activeBoardId)}
        activeMembers={activeBoardMembers}
      ...
      {taskToDelete && (
        <div
          className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm transition-opacity"
          onClick={() => setTaskToDelete(null)}
        >
          <div
            className="w-full max-w-sm transform rounded-xl bg-white p-6 shadow-2xl transition-all dark:bg-slate-800 dark:border dark:border-slate-700"
          ...
          >
            ...
            <h3 className="mb-2 text-center text-lg font-bold text-slate-900 dark:text-white">
              {t?.deleteTaskTitle || "Xóa công việc"}
            </h3>
            ...
            <div className="flex space-x-3">
              <button
                type="button"
                onClick={() => setTaskToDelete(null)}
                className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-[14px] font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600 transition-colors"
              >
              ...
              <button
                ...
                className="flex-1 rounded-lg bg-red-600 px-4 py-2.5 text-[14px] font-semibold text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors"
              >
      ...
      {memberToRemove && (
        <div
          className="fixed inset-0 z-[100000] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm transition-opacity"
      ...
      {transferAdminId && (
        <div
          className="fixed inset-0 z-[100000] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm transition-opacity"
          onClick={() => setTransferAdminId(null)}
        >
          <div
            className="w-full max-w-sm transform rounded-xl bg-white p-6 shadow-2xl transition-all dark:bg-slate-800 dark:border dark:border-slate-700"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <p className="mb-6 text-center text-[14px] text-slate-500 dark:text-slate-400">
              Bạn có CHẮC CHẮN chuyển quyền cho{" "}
              <strong className="text-slate-800 dark:text-slate-200">
                {allUsers.find((u) => u.id === transferAdminId)?.name ||
                  "người này"}
              </strong>
      ...
      <main className="flex-1 flex flex-col min-w-0 transition-all duration-500 bg-slate-100 dark:bg-[#0E1116]">
        {currentView === "home" ? (
          <HomeDashboard
            ...
            allUsers={allUsers}
            currentUser={currentUser}
            navigateToBoard={(id: string, taskId?: string) => {
              if (taskId) {
          ...
          <>
            ...
            <header className="h-14 flex-shrink-0 border-b-2 border-b-slate-400 flex items-center justify-between px-6 bg-white dark:bg-[#0E1116] dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <h2 className="font-semibold text-[15px] truncate max-w-[200px] md:max-w-[300px] text-slate-800 dark:text-slate-100 leading-tight">
                    {boards.find((b) => b.id === activeBoardId)?.name}
                  </h2>
                  {boards.find((b) => b.id === activeBoardId)?.created_at && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div
                  ...
                  className="flex -space-x-2 mr-3 cursor-pointer hover:scale-105 transition-transform"
                  title={t.memberBtn}
                >
                  {activeBoardMembers.slice(0, 3).map((m, idx) => (
                    <div
                      key={`${m.id}-${idx}`}
                      className="w-7 h-7 rounded-full border-2 border-white dark:border-[#0E1116] overflow-hidden relative shadow-sm"
                      style={{ zIndex: 10 - idx }}
                    >
                      <img
                        src={
                          m.avatar_url ||
                          `https://api.dicebear.com/7.x/notionists/svg?seed=${m.id}`
                        }
                  ...
                  {activeBoardMembers.length > 3 && (
                    <div
                      className="w-7 h-7 rounded-full border-2 border-white dark:border-[#0E1116] bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[9px] font-bold text-slate-500 dark:text-slate-300 relative"
                      style={{ zIndex: 0 }}
                    >
                      +{activeBoardMembers.length - 3}
                    </div>
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-[12px] font-medium text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30 border border-green-200 dark:border-green-800/50 bg-white dark:bg-[#1C1F26] rounded-md transition-all flex items-center gap-1.5 hidden sm:flex"
                >
                  <svg
                    width="14"
                    height="14"
                ...
                <div className="relative hidden sm:block">
                  <button
                    ...
                    className="px-3 py-1.5 text-[12px] font-medium text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1C1F26] rounded-md transition-all hover:shadow-sm flex items-center gap-1.5"
                  >
                    <svg
                      width="14"
                      height="14"
                    ...
                    <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-[#1C1F26] rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 z-[120] overflow-hidden">
                      <div className="py-1">
                        <button
                          ...
                          className="w-full text-left px-4 py-2 text-[13px] text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 transition-colors"
                        >
                ...
                <button
                  onClick={() => setIsMembersModalOpen(true)}
                  className="px-3 py-1.5 text-[12px] font-medium border rounded-md dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                >
            ...
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800/50 flex flex-wrap items-center gap-3 flex-shrink-0">
                        ...
                        searchResults.map((task: any) => {
                          ...
                          return (
                            <div
                              ...
                              className="px-4 py-3 border-b border-slate-50 dark:border-slate-800/50 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer transition-colors group"
                            >
                              <p className="text-[13px] font-medium text-slate-800 dark:text-slate-200 truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                {task.content}
                              </p>
                ...
                >
                  ...
                  {Object.entries(priorityConfig).map(([key, config]: any) => (
                    <option key={key} value={key}>
                      {translatePriority(config.label, lang)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              {(searchTaskQuery ||
                ...
                <button
                  ...
                  className="text-[12px] text-red-500 hover:text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 font-medium px-3 py-2 rounded-lg transition-colors flex items-center gap-1.5 ml-1"
                >
                  <svg
                    width="14"
                    height="14"
              ...
              <div className="ml-auto flex bg-slate-100 dark:bg-[#0E1116] p-1 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium rounded-md transition-all ${taskViewMode === "board" ? "bg-white dark:bg-[#1C1F26] text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"}`}
                >
                  <svg
                    width="14"
                    height="14"
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium rounded-md transition-all ${taskViewMode === "list" ? "bg-white dark:bg-[#1C1F26] text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"}`}
                >
                  <svg
                    width="14"
                    height="14"
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium rounded-md transition-all ${taskViewMode === "calendar" ? "bg-white dark:bg-[#1C1F26] text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200"}`}
                >
                  <svg
                    width="14"
                    height="14"
            ...
            <DndContext
              ...
              onDragEnd={onDragEnd}
            >
              {/* BẮT ĐẦU KIỂM TRA ĐIỀU KIỆN VIEW */}
              {taskViewMode === "calendar" ? (
                // 1. TRƯỜNG HỢP: XEM LỊCH
                <CalendarView
                  tasks={filteredTasks}
                      ...
                      {isAddingColumn ? (
                        <div className="bg-slate-50 dark:bg-[#15171C] p-2 rounded-lg border border-blue-400">
                          <input
                            ...
                            onBlur={handleAddColumnBlur}
                            placeholder={t.newColumnName}
                            className="w-full text-[13px] bg-white dark:bg-[#1C1F26] border border-slate-200 dark:border-slate-700 rounded-md px-3 py-2 outline-none dark:text-white"
                          />
                        </div>
                      ) : (
                        <button
                          onClick={() => setIsAddingColumn(true)}
                          className="w-full flex items-center gap-2 text-[13px] font-medium text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 p-3 rounded-lg hover:bg-slate-100 dark:hover:bg-[#15171C]"
                        >
                          <svg
                            width="14"
                            height="14"
                ...
                // 3. TRƯỜNG HỢP: XEM DANH SÁCH (LIST)
                <div className="flex-1 p-4 md:p-6 overflow-y-auto relative">
                  <div className="max-w-4xl mx-auto space-y-6 pb-10">
                    ...
                    {activeBoardId && isManager && (
                      <div className="pt-2">
                        {isAddingColumn ? (
                          <div className="bg-slate-50 dark:bg-[#15171C] p-3 rounded-xl border border-blue-400 shadow-sm">
                            <input
                              ref={newColumnInputRef}
                              autoFocus
                              value={newColumnTitle}
                          ...
                          <button
                            onClick={() => setIsAddingColumn(true)}
                            className="w-full flex items-center justify-center gap-2 text-[13px] font-medium text-slate-500 hover:text-slate-800 p-3.5 rounded-xl hover:bg-slate-100 dark:hover:bg-[#15171C] border border-dashed border-slate-300 dark:border-slate-700"
                          >
                            <svg
                              width="16"
                              height="16"
              ...
              {/* KẾT THÚC KIỂM TRA ĐIỀU KIỆN VIEW */}
                ...
                {activeItem?.type === "Column" ? (
                  <div className="bg-slate-50 dark:bg-[#15171C] p-3 rounded-xl w-[260px] md:w-[280px] shadow-2xl border-2 border-blue-500 scale-95 opacity-90 rotate-2 cursor-grabbing transition-transform duration-200">
                    <div className="flex items-center justify-between">
                      <h3 className="font-semibold text-[14px] text-blue-600 dark:text-blue-400">
                  ...
                  taskViewMode === "board" ? (
                    ...
                    <div className="bg-white dark:bg-[#1C1F26] flex items-center gap-3 p-3 rounded-lg shadow-2xl border border-blue-500 scale-[1.02] w-[400px]">
                      <div
                        className={`w-2 h-2 rounded-full flex-shrink-0 ${priorityConfig[activeItem.data.priority || "normal"].dotClass}`}
                      ></div>
